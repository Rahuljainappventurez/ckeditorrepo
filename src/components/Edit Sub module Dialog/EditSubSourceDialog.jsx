...
function EditSubSourceDialog({ open, setOpen, dataForEdit, subSourceArrayForEdit, setSubSourceArrayForEdit }) {
    ...
    const initialValues = {
        sub_source_name: dataForEdit?.name ?? '',
    }

    const editSubTopicSchema = Yup.object({
        sub_source_name: Yup.string()
            .required("Please enter the Sub Source Name to move ahead"),
    });
    ...
    const handleSave = (values) => {
        const updatedSubSources = subSourceArrayForEdit.map(subSource =>
            subSource._id === dataForEdit._id
                ? { ...subSource, name: values?.sub_source_name }
                : subSource
        );
        setSubSourceArrayForEdit(updatedSubSources);
        handleClose();
    }
                    ...
                    <form onSubmit={handleSubmit} className='edit-sub-topic-file-upload-container'>
                        <Grid className='add-sub-topic-screen-input-container'>
                            ...
                            <InputBox
                                ...
                                id={'edit-sub-source-sub_source_name'}
                                type={'text'}
                                handleChange={handleChange}
                                handleBlur={handleBlur}
                                value={values.sub_source_name}
                            />
                            {errors.sub_source_name && touched.sub_source_name ? (
                                <Grid style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', height: '1rem', width: '100%', marginTop: `${(errors.sub_source_name.length <= 60) ? '-5px' : '8px'}` }}><p style={{ margin: '0', padding: '0' }} className="form-error">{errors.sub_source_name}</p></Grid>
                            ) : null}
                        </Grid>
...
EditSubSourceDialog.propTypes = {
    open: PropTypes.bool.isRequired,
    setOpen: PropTypes.func.isRequired,
};

export default React.memo(EditSubSourceDialog)
