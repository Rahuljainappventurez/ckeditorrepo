...
import { tableSourceChapterManagementAllColumns } from 'constants/TableConstants'
import { tableSourceChapterDummyData } from 'constants/TableConstants'
...
const SourceChapterManagement = () => {
    ...
    const [sourceChapterRowData, setSourceChapterRowData] = useState('');
    const sourceChapterRowDataCallback = (rowData) => {
        setSourceChapterRowData(rowData)
    }
    ...
    const ActionData = [
        {
            label: 'View',
            icon: <Visibility />,
            navigationAddress: routerConstants?.viewSourceChapterRoute,
        },
        {
            label: 'Edit',
            icon: <Edit />,
            navigationAddress: routerConstants?.editSourceChapterRoute,
        },
        {
            label: 'Delete',
            icon: <Delete />,
            ...
            callback: sourceChapterRowDataCallback,
        },
    ]

    // ********* for fetch the source list and transform data according to requirement *********

    const transormSourceChapterData = (data) => {
    ...
    const fetchSourceChapterList = () => {
        return getRequest(apiurl?.SOURCE_CHAPTER_LIST_URL, {
            page: currentPage,
            size: rowsPerPage,
            search: searchTerm,
        })
    ...
    const { isLoading, data: sourceChapterList, isError, error } = useQuery(
        ['source-chapter-management-list', currentPage, rowsPerPage,searchTerm],
        fetchSourceChapterList,
        {
            select: (data) => {
    ...
    useEffect(() => {
        setTotalRecords(sourceChapterList?.totalCount ?? '')
    }, [sourceChapterList])

    // ************* handle refetch after delete ************
    const queryClient = useQueryClient();
    const handleRefetch = () => {
        queryClient.invalidateQueries('source-chapter-management-list');
    };
        ...
        <>
            <Grid className='page-main-container-layout'>
                ...
                <Grid className='table-wrapper-with-text'>
                    ...
                    <CustomAdvanceTable
                        ...
                        allColumns={tableSourceChapterManagementAllColumns}
                        tableData={transormSourceChapterData(sourceChapterList) ?? []}
                        isLoading={isLoading}
...

            {
                ...
                <ConfirmationWithCaptcha
                    ...
                    apiurl={`${apiurl?.SOURCE_CHAPTER_DELETE_URL}/${sourceChapterRowData?._id}`}
                    apiSuccessMsg={toasterMessage?.DELETE_SOURCE_CHAPTER_SUCCESS}
...
export default SourceChapterManagement